...
系統每天整合最新的庫存、採購、進貨、生產計畫與 BOM 資料，逐日推演未來三個月的原料進貨、耗用與庫存變化。

運籌單位不再只能查看當前庫存，而能透過趨勢報表掌握預計進貨、未來耗用及安全水位，提前辨識可能發生缺口的原料與日期。

<img src="./dashboard/01_inventory_forecast_trend.jpg" alt="庫存預測趨勢" width="900">

*庫存預測趨勢：將目前庫存、預計進貨、預估耗用與安全水位呈現在同一條時間軸上，協助判斷未來可能發生缺料的時間點。*
...
報表先彙整需要關注的原料及風險等級，再進一步列出近期缺料日期，使使用者能依處理時效安排追料與排程調整的優先順序。

<table>
  <tr>
    <td align="center" width="35%">
      <img src="./dashboard/02_inventory_volume_alert.jpg" alt="庫存異常警示" width="250"><br>
      <sub>庫存異常警示：彙整需要優先處理的原料與風險等級。</sub>
    </td>
...
部分廢鋼原料進貨後，仍須經過試熔、成分確認及解管程序，不能立即投入生產。若只檢視帳面庫存，可能出現數量充足，但實際投料時原料尚未放行的情況。

系統因此同時建立「總庫存」與「可用庫存」兩種管理視角，並分別設置警示條件。當發現異常時，使用者可進一步查看每日進貨、耗用與庫存推演明細，確認缺口的形成時間與原因。

<img src="./dashboard/04_daily_forecast_matrix.jpg" alt="每日預估明細" width="900">

*每日預估明細：按日呈現進貨、耗用與庫存結果，支援缺料原因追查及後續處理。*
