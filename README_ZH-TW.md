...
> 最低成本 BOM 讓原料配置能隨成本條件動態調整，但也提高了生產對個別原料供應的敏感度。本系統整合庫存、採購、進貨、生產計畫與 BOM 資料，逐日推演未來的原料供需與可用庫存，協助運籌單位在實際投料前辨識斷料風險，提前追料或調整生產計畫。

## 目的

最低成本 BOM 的落地，不只取決於能否計算出成本較低的投料組合，也取決於各項原料能否在預定投料時點實際使用。當原料需求隨 BOM 動態變化，運籌單位需要從掌握當前庫存，進一步預見未來的供需缺口與可處理時間。
