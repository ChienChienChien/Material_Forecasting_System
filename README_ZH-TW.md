...
與 IT 單位協作，將 MES、WAP 中的庫存、採購與進貨資料整合至 Data Warehouse，再以 Python 實作進耗存的業務邏輯，建立原料進耗存的推估模型。
Python 程式每天取得最新資料，依日期順序加入預計進貨、扣除預估耗用，滾動推演未來三個月的每日庫存變化，最後將結果寫入SQL資料庫中。
...
| 能力 | 使用技術 | 專案用途 |
|---|---|---|
| 建模與實作商業邏輯 | Python | 將進貨、耗用及原料放行規則轉換為逐日供需推演 |
